    ...
    apiGet('/api/hero', { cacheKey: 'hero' })
      .then((data) => {
        setHeroData((prev) => ({
          ...
          keyProducts: data.keyProducts?.length ? data.keyProducts : prev.keyProducts,
        }));
      })
    ...
    <>
      ...
      <section className="bg-gray-100 py-10 px-4 md:px-6">
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
          {(keyProducts || []).map((product, index) => (
            <div key={product.title || index} className="bg-white rounded-xl shadow-md overflow-hidden">
              <img
                src={resolveImage(product.image, imageMap, index === 0 ? img1 : img2)}
                alt={product.title}
      ...
      <section className="bg-white py-10 md:py-12 text-center px-4 md:px-6">
        <h2 className="text-2xl md:text-3xl font-bold mb-6">
          {whyChoose?.title || 'Why Choose Greenvora Exim'}
        </h2>
